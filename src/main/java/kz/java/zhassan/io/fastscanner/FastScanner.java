...
/**
 * The alternative buffered fast scanner for InputStream in Java.
 * It is based on pure primitives and outperforms java.io.BufferedReader up to 20 times.
 * @author Zhassan
 */
public class FastScanner {

    static final int DEFAULT_BUFF = 1024, EOF = -1, INT_MIN = 48, INT_MAX = 57, NOT_SET = -1, POS = 1;
    ...
    /**
     * Method for obtaining next available 32 bit integer from the input.
     * Not that it just skips all the non numeric characters around numeric slice and returns the numeric slice.
    ...
    /**
     * Reads all non numeric symbols and assigns the buffer's pointer to the first available numeric character byte
    ...
    private int readNonNumerics() throws IOException {
        ...
        if(signByte == NEG) return -POS;
        return POS;
    }
    /**
     * Closes the input stream passed as parameter to a constructor.
     * We recommend to avoid calling this method explicitly if not necessary!
     ...
     * @throws IOException if some exception occurs while reading bytes into the buffer array.
     */
